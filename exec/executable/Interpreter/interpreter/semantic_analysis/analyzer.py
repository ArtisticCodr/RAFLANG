...
class SemanticAnalyzer(NodeVisitor):
    ...
    class CType(object):
        types = dict(char=int, int=int, float=float, double=float, str=str)
    ...
    def visit_Program(self, node):
        ...
        if not self.current_scope.lookup('opening'):
            self.error(
                "Error: Undeclared mandatory function opening"
            )
    ...
    def visit_Assign(self, node):
        ...
        if left != right:
            self.error("Incompatible types when assigning to type <{}> from type <{}> at line {}".format(
                left,
                right,
                node.line
            ))
    ...
    def visit_FunctionCall(self, node):
        ...
        if func_symbol.params != None:
            if len(node.args) != len(func_symbol.params):
                self.error(
                    "Functio {} takes {} positional arguments but {} were given at line {}".format(
                        func_name,
                        len(node.args),
